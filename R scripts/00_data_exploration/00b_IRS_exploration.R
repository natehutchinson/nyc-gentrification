...
files <- list.files('data/raw_data/irs_raw_data')
...
for (file in files) {
  yr = substr(file,0,2)
  data <- read_csv(paste('data/irs_raw_data/', file, sep = ""), col_types = cols(zipcode = "c")) %>%
    clean_names() %>%
...
irs_data_wide <- pivot_wider(irs_data, id_cols = zipcode, names_from = year, values_from = avg_agi) %>%
  write_csv('data/processed_data/irs_data_clean.csv')
...
## load nyc metro zips
nyc_metro_zips <- read_csv('data/prcoessed_data/nyc_metro_zips.csv')
...
## load nyc zips
nyc_zips <- read_csv('data/processed_data/nyc_zips.csv')
