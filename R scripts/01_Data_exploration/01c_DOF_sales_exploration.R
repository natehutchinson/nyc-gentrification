...
dof_bulk <- read_csv('data/dof_annual_sales.csv') %>%
  clean_names()
...
dof_additional <- read_csv('data/dof_2021_2022/2021_bronx.csv') %>%
  clean_names()

for (file in files[2:length(files)]) {
  temp <- read_csv(paste('data/dof_2021_2022/', file, sep = "")) %>% clean_names
