...
## Description: Import and clean the IRS Tax Statistics data, prepare for merge
...
for (file in files) {
  yr = substr(file,0,2)
  data <- read_csv(paste('data/raw_data/irs_raw_data/', file, sep = ""), col_types = cols(zipcode = "c")) %>%
    clean_names() %>%
